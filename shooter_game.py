...
text_lost2 = font24.render("ПРОИГРЫШ!",True,(255,0,0))
...
class UFO(GameSprite):
	...
	def update(self):
		...
		if self.rect.y>win_height:
			lost += 1#увеличиваем lost на 1
			text_lost = font14.render("Пропущено:"+str(lost),True,(255,0,0))#рендерим снова
			self.rect.y= 0
...
while True:
	...
	for i in hits:
		kills += 1#Увеличиваем kills на 1
		text_killed = font14.render("Убито:"+str(kills),True,(255,0,0))#СНова рендерим надпись
		enemy = UFO(img="ufo.png",w=50,h=50,x=randint(0,win_width-50),y=0,speed=1)
		enemies.add(enemy)
	if kills >= 10:
		...
		quit()
	if lost >= 10:
		window.blit(text_lost2, (win_width//2, win_height//2))
		display.update()
		sleep(2)
		quit()
	for i in event.get(): 
